...
The API ships pre-computed semantic cross-references for three of the four cross-corpus query patterns:

| Use case | How to query |
|---|---|
| User input → UB paragraphs | `POST /search/semantic` (live, `text-embedding-3-small` + HNSW) |
| **UB paragraph → UB paragraphs** | `GET /paragraphs/{ref}?include=paragraphParallels` |
| **UB paragraph → Bible verses** | `GET /paragraphs/{ref}?include=bibleParallels` |
| **Bible verse → UB paragraphs** | `GET /bible/{book}/{chapter}/{verse}/paragraphs` |

The bottom three are pre-computed at seed time using `text-embedding-3-large` (3072-d) — every response is a fast index lookup. The first stays on the live HNSW path with `text-embedding-3-small` since query text is unbounded.

You can combine includes: `?include=entities,bibleParallels,paragraphParallels` returns all three on a single round-trip.

### UB paragraph → UB paragraphs

```bash
curl 'https://api.urantia.dev/paragraphs/1:0.1?include=paragraphParallels'
```
