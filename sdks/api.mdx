...
// Vector embeddings (default: text-embedding-3-large 3072-d)
const { data: vec } = await api.embeddings.get('2:0.1')
const { data: vecSmall } = await api.embeddings.get('2:0.1', { model: 'small' })

// Bible — UB↔Bible cross-references
const { data: bibleHits } = await api.bible.semanticSearch({
  q: 'love your enemies',
  limit: 5,
  urantiaParallelLimit: 3,
})
const { data: para } = await api.paragraphs.get('1:0.1', {
